$(function() {
    ...
    $(".J_menuItem").each(function(k) {
        if (!$(this).attr("data-index")) { $(this).attr("data-index", k) }
    });
    ...
    function f(l) {
        var k = 0;
        $(l).each(function() { k += $(this).outerWidth(true) });
        return k
    }
    ...
    function b() {
        ...
        if ($(".page-tabs-content").width() < k) {
            ...
            p = f($(m).prevAll());
            if (p > 0) { $(".page-tabs-content").animate({ marginLeft: 0 - p + "px" }, "fast") }
        }
    }
